...
# python create_dataset.py --cell_line gm12878 --save_path ./data/dataset/ --test
...
def save_data(chroms, target_list, feat_list, dnase_list, map_list, path, flank):
    if len(target_list) == 0:
        for atac_seq, dnase, map_seq in zip(feat_list, dnase_list, map_list):
            X, meta, indexing, X_dnase, X_map = make_input_target_from_list(chroms, None, atac_seq, dnase, map_seq, flank)
        ...
        for chrom_data_dict, atac_seq, dnase, map_seq in zip(target_list, feat_list, dnase_list, map_list):
            X, meta, Y, indexing, X_dnase, X_map = make_input_target_from_list(chroms, chrom_data_dict, atac_seq, dnase, map_seq, flank)
...
def generate(atac_path, hic_path, save_dir, mode, seq_len=401, chrs=None):
    ...
    cell_line = (atac_path.rstrip('/').split('/')[-1]).split('_')[0]
    ...
    save_data(chroms, target_list, feat_list, dnase_list, map_list, save_path, flank)
